...
if command -v python3 >/dev/null 2>&1; then
    # Markdown (.md/.markdown) is out of Q-13's character scope by rule: it's
    # prose docs with a deliberate house style (see .claude/rules.md Q-13
    # "Scope"), so em-dashes etc. are allowed there. This also covers the
    # rules / memory files that quote the offending characters as examples.
    # Carrier files - the precommit script itself + the workflow YAML - quote
    # the detection patterns legitimately, so they're excluded too.
